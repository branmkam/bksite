      ...
      <div className="grid grid-cols-1 gap-2 md:grid-cols-2 lg:grid-cols-3 ">
        {timeCatalog.map((c) => (
          <div
            key={"div" + c.name}
            className={
              `min-w-[270px] max-w-[380px] h-[350px] border-yellow-500 m-2 bg-slate-900 p-2 rounded-lg flex flex-col items-center justify-between text-center bg-[url(${c.image})]`
            }
          >
            ...
            {c.collab_artists.length > 0 ? (
              c.collab_artists.map((a) => (
                <span className="text-sm" key={c.name + "artist" + a.name}>
                  ft.{" "}
                  <a href={a.link} target="_blank" rel="noreferrer" className="text-yellow-500 hover:text-yellow-200">
                    {a.name}
                  </a>
                </span>
              ))
