...
export default function Countdown(props) {
  ...
  let finstr = days + ':' + zerofy(hours) + ':' + zerofy(mins) + (seconds && ':' + zerofy(secs))

  return time < current ? <p className={className}>00:00:00{seconds && ':00'}</p>: <p className={className}>{finstr}</p>;
}
